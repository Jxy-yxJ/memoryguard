...
def delta_vs_baseline(
    ...
    method_policy: str = "verify_rearrangement_risk",
...
def write_outputs(rows: Sequence[Dict[str, float | int | str]], out_dir: Path) -> None:
    ...
    _write_csv(out_dir / "ai2thor_rearrangement_multiseed_summary.csv", rows)
    _write_csv(out_dir / "ai2thor_rearrangement_multiseed_delta.csv", deltas)
    (out_dir / "ai2thor_rearrangement_multiseed.json").write_text(
...
def render_readme(rows: Sequence[Dict[str, float | int | str]], deltas: Sequence[Dict[str, float | int | str]]) -> str:
    ...
    lines = [
        "# AI2-THOR Rearrangement Multi-Seed Summary",
        "",
        f"Rows: `{len(rows)}`",
        "",
    ]
    if best_delta:
        lines.extend(
            [
                f"Best completion delta vs salience: budget `{best_delta['budget']}`",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Aggregate AI2-THOR rearrangement verification summaries across seeds.")
    parser.add_argument("summary_paths", type=Path, nargs="+")
    parser.add_argument("--out-dir", type=Path, default=Path("results/ai2thor_rearrangement_multiseed"))
